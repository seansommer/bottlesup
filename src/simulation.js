...
export class Simulation{
 ...
 loadBin(){if(this.ended||this.paused||this.binState!=='empty'||this.tilt>8||this.binsLoaded>=this.binsRequired)return false;this.binState='loading';this.loadTime=1.2;this.event('load');return true;}
 setLift(dir){this.lift=Math.sign(dir);}
 setFeeder(value){this.feeder=Math.max(0,Math.min(1,Number(value)||0));}
 stopBelt(){if(!this.cards||this.ended||this.paused)return false;this.cards--;this.stopUntil=Math.max(this.time,this.stopUntil)+7;this.event('stop');return true;}
 ...
 action(id,reject=false,helper=false){if(this.ended||this.paused)return false;const b=this.bottles.find(b=>b.id===id);if(!b||b.belt!=='secondary')return false;
 ...
 openReward(){const kind=this.pendingReward;if(!kind||this.ended||this.paused)return null;this.pendingReward=null;if(kind==='stop')this.stopUntil=Math.max(this.time,this.stopUntil)+8;if(kind==='helper'){if(this.helpers.length<4)this.helpers.push({until:this.time+25,next:this.time+.6,index:this.helpers.length});else this.points(250);}if(kind==='points')this.points(250);if(kind==='card')this.cards=Math.min(5,this.cards+1);if(kind==='quality')this.qualityUntil=this.time+18;this.event('reward',{kind});return kind;}
 step(dt){if(this.paused||this.ended)return;dt=Math.min(.05,Math.max(0,dt));this.time+=dt;
  ...
  if(this.binState==='ready'&&this.tilt>42&&this.binLeft){this.dumpCredit+=dt*(1.2+(this.tilt-42)*.19);while(this.dumpCredit>=1&&this.binLeft){this.dumpCredit--;this.binLeft--;if(this.bottles.filter(b=>b.belt==='primary').length>=RULES.primaryCapacity){this.waste++;this.points(-35);this.event('waste',{why:'feeder overflow',x:-4,z:-4});if(this.mode!=='practice'&&this.waste>=RULES.wasteLimit){this.finish('Feeder overflow');return;}}else this.addBottle();}if(!this.binLeft){this.binState='empty';this.dumpCredit=0;this.event('binEmpty');}}
  ...
  if(this.binsLoaded>=this.binsRequired&&this.binLeft===0&&this.bottles.length===0&&!this.ended){this.level++;this.binsLoaded=0;this.binState='empty';this.tilt=0;this.lift=0;this.points(300);this.event('level',{level:this.level,juice:this.juice.name});}
 }
