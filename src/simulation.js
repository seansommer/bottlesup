...
export class Simulation{
 ...
 get queueCount(){return intakeReady(this.secondary).length;}
 ...
 step(dt){if(this.paused||this.ended||this.batchReady)return;dt=Math.min(.05,Math.max(0,dt));this.time+=dt;
  ...
  for(const b of primary){
   b.age+=dt;const peers=primary.filter(o=>(o.layer||0)===(b.layer||0));moveBody(b,0,this.feeder*2.6*dt,peers,PRIMARY_BOUNDS);
   if(b.layer>0&&fits(b,primary.filter(o=>(o.layer||0)===b.layer-1),PRIMARY_BOUNDS))b.layer--;b.y=1.67+(b.layer||0)*.48;
  }
  this.transferCredit=Math.min(2,this.transferCredit+dt*this.feeder*9);
  for(const b of primary){
   if(this.transferCredit<1)break;if(b.z+footprint(b).extentZ<PRIMARY_BOUNDS.maxZ-.025)continue;
   const place=transferPosition(b,this.secondary);if(!place)continue;
   Object.assign(b,place,{belt:'secondary',age:0,layer:0});this.transferCredit--;
  }
  const secondary=this.secondary;moveSecondary(secondary,this.stopped?0:this.secondarySpeed,dt);
  for(const b of secondary){
   b.age+=dt;
   if(b.finicky&&!b.wobbled&&b.up&&b.age>b.wobbleAt){
  ...
  if(ready.length===6)this.intake(ready,false);
  else if(ready.length&&this.binsLoaded>=this.binsRequired&&!this.binLeft&&!this.bottles.some(b=>b.belt!=='secondary')&&ready.length===this.secondary.length)this.intake(ready,true);
